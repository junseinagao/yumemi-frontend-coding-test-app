...
describe("HomePage", () => {
  ...
  it("「都道府県」と書かれたセクションタイトルがレンダリングされているか", async () => {
    const sectionTitle = await screen.findByText("都道府県")
    expect(sectionTitle).toBeInTheDocument()
  })
  it("全ての都道府県名とチェックボックスがレンダリングされているか", async () => {
    const todoufukenMockData = [
      "北海道",
      "青森県",
      "岩手県",
      "宮城県",
    ...
    for (let i = 0; i < todoufukenMockData.length; i++) {
      const name = todoufukenMockData[i]
      const checkboxInput = await screen.findByLabelText(name)
